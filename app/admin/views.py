...
def articles():
    ...
    articles = Article.query.filter(
        ...
        ).order_by(Article.timestamp.desc()).paginate(page, per_page=current_app.config['FLASKBLOG_POST_PER_PAGE'], error_out=False)
...
def write():
    ...
    editor = request.args.get('editor', current_app.config.get('FLASKBLOG_EDITOR'))
    form.editor.data = editor
    if editor == 'tinymce':
        return render_template('admin/write_tinymce.html',form=form)
    return render_template('admin/write.html', form=form)
...
def draw_preview():
    ...
    title = request.args.get('title','flaskBlog')
...
def upload():
    ...
    if not allowed_file(file.filename):
        ...
        url_path = ''
        upload_type = current_app.config.get('FLASKBLOG_UPLOAD_TYPE')
        ex=os.path.splitext(file.filename)[1]
        filename=datetime.now().strftime('%Y%m%d%H%M%S')+ex
        if upload_type is None or upload_type == '' or upload_type == 'local':
            file.save(os.path.join(current_app.config['FLASKBLOG_UPLOAD_PATH'],filename))
...
def categorys():
    ...
    """
    page = request.args.get('page', 1, type=int)
    categorys = Category.query.order_by(Category.id.asc()).paginate(page, per_page=current_app.config['FLASKBLOG_POST_PER_PAGE'], error_out=False)
...
def get_image(filename):
    return send_from_directory(current_app.config['FLASKBLOG_UPLOAD_PATH'], filename)
...
def baidu_push_article():
    ...
    domain = current_app.config.get('FLASKBLOG_DOMAIN','https://www.FLASKBLOG.com')
...
def settings():
    ...
    for name in form._fields:
        s = setting_dict.get(name, None)
        if s :
            if name in ['flaskblog_comment','flaskblog_register_invitecode']:
