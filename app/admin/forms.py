...
class SettingForm(FlaskForm):
    flaskblog_domain = StringField('网站域名', default='www.flaskblog.com', validators=[DataRequired()])
    flaskblog_title = StringField('网站名称', default='flaskBlog', validators=[DataRequired()])
    flaskblog_keywords = StringField('关键字', default='python blog')
    flaskblog_description = TextAreaField('网站描述')
    
    flaskblog_comment = BooleanField(label='开启评论',default = False)
    flaskblog_register_invitecode = BooleanField(label='开启邀请注册', default = False)
    flaskblog_editor = SelectField('默认编辑器', choices=[('markdown', 'markdown'), ('tinymce', 'tinymce')], default = 'markdown')
    flaskblog_template = SelectField('模板')

    flaskblog_upload_type = SelectField('图片存储方式', choices=[('local','本地'),('qiniu', '七牛云')])
    flaskblog_allowed_image_extensions = StringField('运行图片后缀', default='png, jpg, jpeg, gif, webp')
    ...
    flaskblog_tongji_script = TextAreaField('网站统计代码', default='')
    flaskblog_extend_meta = TextAreaField('网站META', default= '')
    flaskblog_robots = TextAreaField('robots', default='User-agent: *\nAllow: /')
    ...
    def __init__(self,*args,**kwargs):
        ...
        tpls = [ (t,t) for t in os.listdir(base_path)]
        self.flaskblog_template.choices = tpls
