...
def register_template_filter(app):
    ...
    def hidden_content(content):
        if current_user.is_authenticated:
            return content.replace('[flask_hidden]','').replace('[/flask_hidden]','')
        else:
            ...
            '''.format(login_url)
            return re.sub('\[flask_hidden\].*?\[/flask_hidden\]',repl,content,flags=re.DOTALL)
...
def register_template_global(app: Flask):
    ...
    def get_articles(
            ...
            tags:str = None, # 文章标签，文章标签逗号分割比如"python,安全,flaskBlog"
        ...
        if categorys and len(categorys) > 0 :
            ctys = Category.query.filter(Category.name.in_(categorys.split(','))).all()
            cty_ids = [c.id for c in ctys]
        query = Article.query.filter(Article.state == 1)
        if cty_ids :
            query = query.filter(Article.category_id.in_(cty_ids))
    ...
    @app.template_global()
    def get_categorys(names:str = None) -> List[Category] :
        """
        获取文章分类
        """
        query = Category.query.filter(Category.visible == True)
        if names and len(names) > 0:
            query = query.filter(Category.name.in_(names.split(',')))
        query.order_by(Category.sn.asc())
        return query.all()
